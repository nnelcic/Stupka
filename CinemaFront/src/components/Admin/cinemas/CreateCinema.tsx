...
const CreateCinema: React.FC<CreateCinemaFormProps> = ({ setOccuredError, setShowError, close, setRerender }) => {
    ...
    const { createCinema } = useCinemas();
    
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        event.preventDefault();
        try {
            createCinema(email, address, phoneNumber, city, name, setShowError, setOccuredError);
            close();
            setTimeout(() => {
                setRerender(x => !x)
            }, 1000);
